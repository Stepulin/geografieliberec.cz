...
shapefile = '/root/data/countries_110m/ne_110m_admin_0_countries.shp'
datafile = '/root/datafinal2.csv'
gdf = gpd.read_file(shapefile)[['ADMIN', 'ADM0_A3', 'geometry']]
gdf.columns = ['country', 'country_code', 'geometry']
df = pd.read_csv(datafile)
df_2019 = df[df['midperiod'] == 1953]
...
palette = brewer["Purples"][4]
palette = palette[::-1]
color_mapper = LinearColorMapper(palette = palette, low = 0, high = 40, nan_color = '#d9d9d9')
tick_labels = {"0": "0", "10": "10", "20":"20", "30":"30", "40":"40+"}
color_bar = ColorBar(color_mapper = color_mapper, label_standoff = 8, width = 500, height = 20,
                     border_line_color = None, location = (0,0), orientation = "horizontal", major_label_overrides = tick_labels)
p = figure(title = 'Porodnost v roce 2019', plot_height = 720, plot_width = 1280, toolbar_location = "right")
p.xgrid.grid_line_color = None
p.ygrid.grid_line_color = None
p.patches('xs','ys', source = geosource,fill_color = {'field' : 'porodnost', 'transform' : color_mapper}, line_color = 'black', line_width = 0.25, fill_alpha = 1)
...
hover = HoverTool(tooltips = [('Stát','@country'),('Porodnost.', '@porodnost')])
p = figure(title = 'Porodnost v roce 2019', plot_height = 720, plot_width = 1280, toolbar_location = "right", tools = [hover, ResetTool(), BoxSelectTool(), PanTool(), SaveTool()])
...
def update_plot(attr, old, new):
    ...
    p.title.text = 'Porodnost v roce %d' %yr
slider = Slider(title = 'Rok',start = 1953, end = 2098, step = 1, value = 1953)
