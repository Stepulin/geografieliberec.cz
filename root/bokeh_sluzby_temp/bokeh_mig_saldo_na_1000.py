...
palette = brewer["RdYlBu"][6]
palette = palette[::-1]
color_mapper = LinearColorMapper(palette = palette, low = -150, high = 150, nan_color = '#d9d9d9')
tick_labels = {"-150": "-150", "-50": "-50","0":"0", "50":"50", "100":"100", "150":"150"}
color_bar = ColorBar(color_mapper = color_mapper, label_standoff = 8, width = 500, height = 20,
                     border_line_color = None, location = (0,0), orientation = "horizontal", major_label_overrides = tick_labels)
p = figure(title = 'Migrační saldo na 1 000 obyvatel v roce X', plot_height = 720, plot_width = 1280, toolbar_location = "right")
p.xgrid.grid_line_color = None
p.ygrid.grid_line_color = None
p.patches('xs','ys', source = geosource,fill_color = {'field' : 'mig_saldo_na_1000', 'transform' : color_mapper}, line_color = 'black', line_width = 0.25, fill_alpha = 1)
...
hover = HoverTool(tooltips = [('Stát','@country'),('Migrační saldo na 1 000 obyvatel', '@mig_saldo_na_1000')])
p = figure(title = 'Migrační saldo na 1 000 obyvatel v roce X', plot_height = 720, plot_width = 1280, toolbar_location = "right", tools = [hover, ResetTool(), BoxSelectTool(), PanTool(), SaveTool()])
...
def update_plot(attr, old, new):
    ...
    p.title.text = 'Migrační saldo na 1 000 obyvatel v roce %d' %yr
