...
p = figure(title = 'Celkový počet mužů v roce 2019', plot_height = 720, plot_width = 1280, toolbar_location = "right")
p.xgrid.grid_line_color = None
p.ygrid.grid_line_color = None
p.patches('xs','ys', source = geosource,fill_color = {'field' : 'pocet_ob_muzi', 'transform' : color_mapper}, line_color = 'black', line_width = 0.25, fill_alpha = 1)
...
color_bar = ColorBar(color_mapper = color_mapper, label_standoff = 8, width = 500, height = 20,
                     border_line_color = None, location = (0,0), orientation = "horizontal", major_label_overrides = tick_labels)
hover = HoverTool(tooltips = [ ('Stát','@country'),('Počet ob.', '@pocet_ob_muzi')])
p = figure(title = 'Celkový počet mužů v roce 2019', plot_height = 720, plot_width = 1280, toolbar_location = "right", tools = [hover, ResetTool(), BoxSelectTool(), PanTool(), SaveTool()])
...
def update_plot(attr, old, new):
    ...
    p.title.text = 'Celkový počet mužů v roce %d' %yr
slider = Slider(title = 'Rok',start = 1950, end = 2019, step = 1, value = 1950)
