...
# palette = palette[::-1]
color_mapper = LinearColorMapper(palette = palette, low = -20, high = 20, nan_color = '#d9d9d9')
tick_labels = {"-20":"-20 %", "-15":"-15 %", "-10":"-10 %", "-5":"-5 %", "0":"0 %", "5":"5 %", "10":"10 %", "15":"15 %", "20":"20 %"}
color_bar = ColorBar(color_mapper = color_mapper, label_standoff = 8, width = 700, height = 20,
                     border_line_color = None, location = (0,0), orientation = "horizontal", major_label_overrides = tick_labels)
p = figure(title = 'Procentuální nárůst populace v roce X', plot_height = 720, plot_width = 1280, toolbar_location = "right")
p.xgrid.grid_line_color = None
p.ygrid.grid_line_color = None
p.patches('xs','ys', source = geosource,fill_color = {'field' : 'procentualni_narust_pop', 'transform' : color_mapper}, line_color = 'black', line_width = 0.25, fill_alpha = 1)
...
hover = HoverTool(tooltips = [('Stát','@country'),('Procentuální nárůst populace', '@procentualni_narust_pop')])
p = figure(title = 'Procentuální nárůst populace v roce X', plot_height = 720, plot_width = 1280, toolbar_location = "right", tools = [hover, ResetTool(), BoxSelectTool(), PanTool(), SaveTool()])
...
def update_plot(attr, old, new):
    ...
    p.title.text = 'Procentuální nárůst populace v roce %d' %yr
