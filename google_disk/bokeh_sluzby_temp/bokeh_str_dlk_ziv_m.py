...
palette = brewer["RdPu"][5]
palette = palette[::-1]
color_mapper = LinearColorMapper(palette = palette, low = 0, high = 100, nan_color = '#d9d9d9')
tick_labels = {"0": "0", "50": "50", "60":"60", "70":"70", "80":"80", "99":"99+"}
color_bar = ColorBar(color_mapper = color_mapper, label_standoff = 8, width = 500, height = 20,
                     border_line_color = None, location = (0,0), orientation = "horizontal", major_label_overrides = tick_labels)
p = figure(title = 'Střední délka života u mužů v roce X', plot_height = 720, plot_width = 1280, toolbar_location = "right")
p.xgrid.grid_line_color = None
p.ygrid.grid_line_color = None
p.patches('xs','ys', source = geosource,fill_color = {'field' : 'str_dlk_ziv_M', 'transform' : color_mapper}, line_color = 'black', line_width = 0.25, fill_alpha = 1)
...
hover = HoverTool(tooltips = [('Stát','@country'),('Střední délka života - MUŽI', '@str_dlk_ziv_M')])
p = figure(title = 'Střední délka života u mužů v roce X', plot_height = 720, plot_width = 1280, toolbar_location = "right", tools = [hover, ResetTool(), BoxSelectTool(), PanTool(), SaveTool()])
...
def update_plot(attr, old, new):
    ...
    p.title.text = 'Střední délka života u mužů v roce %d' %yr
